...
Serial.println ("in init");
	...
	PCMSK0 |= (1<<PCINT0);
#endif
Serial.println ("about to do while loop");
	while(zg_get_conn_state() != 1) {
		zg_drv_process();
	}
Serial.println ("about to start the stack");
	// Start the stack
	stack_init();
Serial.println ("after starting the stack");

	// Store the callback function for serving pages
	// and start listening for connections on port 80 if
	// the function is non-null
Serial.println ("before setting callbackFuncion");
	callbackFunc = function;
	if (callbackFunc) {
		// Listen for server requests on port 80
		uip_listen(HTONS(80));
	}
Serial.println ("after setting callbackFuncion");
...
/*
* Reset the WiServer/WiFi if the connection has been lost and set active
* to false for all items in the queue
*/
boolean Server::resetOnLostConnection()
{
	// If the current connection is valid exit early (nothing needs to happen)
	...
	wdt_reset ();
	#endif
Serial.println ("in wisheild reset code 1");

	// Set all Requests->active in the queue to false
	GETrequest* r = queue;
	if(NULL != queue) {
		while(r->next != NULL) {
			r->active = false;
	...
	wdt_reset ();
	#endif
Serial.println ("in wisheild reset code 2");
	
	// Call WiServer.init() to reset
	this->init(callbackFunc);
	
	// reset wdt
	#ifdef USE_WDT
	wdt_reset ();
	#endif
Serial.println ("in wisheild reset code 3");
...
void Server::server_task() {
	...
	// Reset the WiServer/WiFi if the connection has been lost and set active
	// to false for all items in the queue
	if(false == resetOnLostConnection()) {
