...
#define INIT_STATE_ZG_INIT 0
#define INIT_STATE_CONNECT 1
#define INIT_STATE_STACK_INIT 2
#define INIT_STATE_COMPLETE 3

/*
 *	Call this repeatedly in your loop() and it will return
 *	0 until the link is up.  When link is up it will return 1
 *
 *	If restart is set to 1 the state machine will reset and
 *	begin the connection from the start.
 */
int Server::async_init(pageServingFunction function, int restart) {
	static int state=INIT_STATE_ZG_INIT;
	if (restart) {
		state = INIT_STATE_ZG_INIT;
	}

	switch(state) {
		case INIT_STATE_ZG_INIT:
			// WiShield init
			zg_init ();
		#ifdef USE_DIG0_INTR
			attachInterrupt (0, zg_isr, LOW);
			...
			PCMSK0 |= (1<<PCINT0);
		#endif
			state = INIT_STATE_CONNECT;
			return (0);
		case INIT_STATE_CONNECT:
			if (zg_get_conn_state() != 1) {
				zg_drv_process ();
			} else {
				state = INIT_STATE_STACK_INIT;
			}
			return(0);
		case INIT_STATE_STACK_INIT:
			// Start the stack
			stack_init ();
			// Store the callback function for serving pages
			// and start listening for connections on port 80
			// if the function is non-null
			callbackFunc = function;
			if (callbackFunc) {
				// Listen for server requests on port 80
				uip_listen (HTONS(80));
