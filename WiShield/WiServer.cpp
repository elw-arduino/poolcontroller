...
/*
	// added by Mohrgan
	return ((0==(int)uip_conn->appstate.ackedCount) &&
		(0==(int)uip_conn->appstate.sentCount) &&
		(0!=uip_conn->appstate.cursor));
*/
	return Sending;
	return false; // FIX ME
}
// added by Mohrgan
boolean Server::firstCall () {
	return ((0==(int)uip_conn->appstate.ackedCount) &&
		(0==(int)uip_conn->appstate.sentCount));
}
...
boolean Server::server_task() {
	...
	if (state == DISCONNECTED) {
		Serial.println("Init MEGA WiShield");
